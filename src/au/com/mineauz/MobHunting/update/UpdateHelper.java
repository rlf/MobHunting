...
public class UpdateHelper {
	...
	public static void hourlyUpdateCheck(final CommandSender sender,
		...
		if (updateCheck) {
			...
			}.runTaskTimer(MobHunting.instance, 0L,
					MobHunting.config().checkEvery * 20L);
			// Check for update timer
		}
	}

	public static void pluginUpdateCheck(final CommandSender sender,
			boolean updateCheck, final boolean silent) {
		if (updateCheck) {
			if (!silent) {
				MobHunting.instance
			...
			if (updateAvailable != UpdateStatus.RESTART_NEEDED) {
				...
				MobHunting.instance
						.getServer()
						.getScheduler()
						.runTaskAsynchronously(MobHunting.instance,
								new Runnable() {
									@Override
									public void run() {
				...
				}.runTaskTimer(MobHunting.instance, 20L, 20L); // Check status
																// every second
			} else {
				sender.sendMessage(ChatColor.GREEN
						+ "[MobHunting]"
	...
	public static boolean downloadAndUpdateJar() {
		try {
			if (updateAvailable != UpdateStatus.RESTART_NEEDED)
				downloadFile(getBukkitUpdate().getVersionLink(),
						"plugins/MobHunting/update/");
			File oldFile = new File("plugins/" + getCurrentJarFile());
			File disabledFile = new File("plugins/" + getCurrentJarFile()
					+ ".old");
			int count = 0;
			while (disabledFile.exists() && count++ < 10) {
				disabledFile = new File("plugins/" + getCurrentJarFile()
						+ ".old" + count);
			}
			if (!disabledFile.exists()) {
				...
				File newJarFile = new File("plugins/MobHunting/update/"
						+ UpdateHelper.getBukkitUpdate().getVersionFileName());
	...
	public static UpdateStatus isUpdateNewerVersion() {
		...
		String[] split = UpdateHelper.getBukkitUpdate().getVersionName()
				.split(" V");
		...
		if (split.length == 2) {
			...
			// CHeck the version #'s
			String[] pluginVer = MobHunting.instance.pluginVersion.split("\\.");
